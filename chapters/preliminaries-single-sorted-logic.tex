...
We use $f,g,h,\dots$ as meta-variables for function symbols and $P,Q,R,\dots$ for predicate symbols.
...
\begin{enumerate}
  ...
  \item the connectives $\lnot$, $\land$, $\lor$ (not, and, or) and
        logical constants $\bot$, $\top$ (false, true);
...
\begin{definition}[$\mathcal{L}$-formulas]\label{def:formulas}
Let $\mathcal{L}$ be a first-order vocabulary.
...
\begin{remark}
    ...
    interpreted as the actual equality, or that a designated ``\(\text{SUCC}(x, y)\)''
    relation holds only if the underlying objects are actual natural numbers,
    for which we have $x + 1 = y$; see e.g.~\autoref{def:fo-arith}.
...
induction on $\varphi$:
\begin{enumerate}
  \item the structure \(\mathcal{M}\) satisfies $\top[\sigma]$ and $\mathcal{M} \not\models \bot[\sigma]$;
  ...
  \]
  \item the structure satisfies $\neg \varphi[\sigma]$ iff $\mathcal{M} \not\models \varphi[\sigma]$;
  \item the structure satisfies $(\varphi \lor \psi)[\sigma]$ iff
        $\mathcal{M} \models \varphi[\sigma]$ or $\mathcal{M} \models \psi[\sigma]$;
  \item the structure satisfies $(\varphi \land \psi)[\sigma]$ iff
        $\mathcal{M} \models \varphi[\sigma]$ and $\mathcal{M} \models \psi[\sigma]$;
  \item the structure satisfies $(\forall x\ldotp \varphi)[\sigma]$ iff
        $\mathcal{M} \models \varphi[\sigma(m/x)]$ for all $m \in M$;
  \item the structure satisfies $(\exists x\ldotp \varphi)[\sigma]$ iff
