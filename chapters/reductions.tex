...
% FL = L* : https://web.archive.org/web/20190320200248/https://core.ac.uk/download/pdf/81978561.pdf
...
Let \(T : \mathbb{N} \to \mathbb{N}\) be some function.  A language \(L\) belongs to \(\complexity{DTIME}(T(n))\) iff there exists a deterministic Turing machine that:
\begin{enumerate}[itemsep=0pt]
    \item\label{itm:dtime} for some \(c > 0\) and every input \(w \in \{0,1\}^{n}\) terminates within \(c \cdot T(n)\) steps;
...
Let \(T : \mathbb{N} \to \mathbb{N}\) be some function.  A language \(L\) belongs to \(\complexity{DSPACE}(T(n))\) iff there exists a deterministic Turing machine that:
\begin{enumerate}
    \item\label{itm:dspace} for some \(c > 0\) and every input \(w \in \{0,1\}^{n}\) terminates,
        at any step using at most \(c \cdot T(n)\) work-tape cells;
    \item for every \(w \in L\) the machine outputs \(1\);
    \item for every \(w \notin L\) the machine outputs \(0\).
\end{enumerate}
