...
We will introduce concepts about P, FP, NP, FNP in the style of~\cites[Theorem1]{10.5555/1540612}[Theorem2]{DRAFT10.5555/1540612}.
...
% Given x, y checking if Problem(x, y) is decidable by poly-time turing machine.

% There are many characterizations of P, and for our purposes they are not equivalent.
% We will study these in~\ref{sec:alternative-ptime}.

% \begin{definition}[Deterministic characterization of \complexity{P}]
% A language $L$ is in \complexity{P} iff there exists a deterministic Turing machine $M$ such that:
% \begin{enumerate}
%     \item $M$ halts on every input within time bounded by a polynomial in the input length.
%     \item For all $x \in L$, $M(x) = 1$.
%     \item For all $x \notin L$, $M(x) = 0$.
% \end{enumerate}
% \end{definition}
...
% \section{Alternative characterizations of P}
% \label{sec:alternative-ptime}
% E.g. in~\cite{bournez_et_al:LIPIcs.MFCS.2019.23}, FP and FNP is characterized by disrete differential equations.
...
% Most problems that are considered in complexity theory ask, for a given input,
% if it satisfies some property or not. I.e.\ the return type of the function that solves
% the problem, is a boolean. Here we list the definitions of some of complexity classes
% that contain solely such decisional problems. These turn out to be the most well-known complexity classes,
% but not the primary interest of this work. Our focus are more general functions that can return
% a string, a pair of numbers, or a graph, and not just a boolean value. After we introduce
% the decisional complexity classes in this section, we will use them to define functional
% complexity classes in~\ref{sec:functional-complexity-classes}.
