...
\section{Decisional versus functional complexity classes}\label{sec:decisional-and-functional-complexity-classes}
...
classes. As we discuss in~\autoref{sec:recursion-theory}, programming-language-like characterizations
...
\begin{definition}[\complexity{FP} (Version 2)]\label{def:fp}
...
\end{enumerate}
\emph{and additionally} there exists a polynomial-time Turing machine (a ``verifier'') that, given an arbitrary
input pair $(x, y)$, decides if $P(x, y)$.


\begin{remark}
  This definition is inspired by~\cite[Section 28.10]{Rich2007Automata}, where it is defined
  without the verifier requirement. However, this requirement is crucial.
  Let's consider two relations. Let it be undecidable if $H(x, y)$
  for given $x, y$.
  Let $R(x, y)$ be such that $R(x, y)~\iff~(H(x, y)\;\lor\;y=0)$. Notice that for every $x$, we can trivially
  find \emph{some} $y$ such that $R(x, y)$. But it is undecidable to test if $R(x, y)$ given an arbitrary
  input pair $(x, y)$. Such a complexity class would not be contained in $\complexity{FNP}$ for
  a sensible definition of that class (which we'll study later).
  
  Our version makes it elegant to compare \(\complexity{FP}\) with \(\complexity{FNP}\) (defined later) ---
...
\begin{remark}[Fine-grained reductions]\label{remark:fine-grained-reductions}
...
\begin{definition}[{\cites[Slide~4]{Goebel2011NashComplexity}[Slide~5]{Goldberg2021SearchTotal}}~Polynomial-time reductions for \complexity{FNP}]\label{def:reductions-for-fnp}
    ...
    \begin{enumerate}
    \item for an arbitrary input $x$ for $\texttt{HardProblem}$,
      if $\texttt{NewProblem}(f(x), y)$, \\ then also $\texttt{HardProblem}(x, g(y))$;
    \item if, for a given input $x$, there exists $y_{\,\texttt{Hard}}$ such that
    $\texttt{HardProblem}(x, y_{\,\texttt{Hard}})$, then there also exists a $y_{\,\texttt{New}}$
    such that $\texttt{NewProblem}(f(x), y_{\,\texttt{New}})$.
    \end{enumerate}

    % In other words, for a given input \(x\) of \(\texttt{HardProblem}\), we can run \(\texttt{NewProblem}(f(x))\)
    % to obtain some result \(y\), such that \(g(y)\) is the result of \(\texttt{HardProblem}(x)\).
\end{definition}

There are plenty of \complexity{NP}-complete problems described in the literature.
However, it is unlikely that we will obtain \complexity{FNP} from taking the closure of an \complexity{NP}-complete problem
under the just defined notion of reductions. The class 
...
\subsection*{Digression: Oracle Turing machines and the technique of forcing}\label{subsec:oracle-forcing}
