...
In~\ref{sec:decisional-complexity-classes} we introduce standard complexity classes such as \(\complexity{L}\), \(\complexity{P}\)
...
As we will see in~\ref{sec:decisional-and-functional-complexity-classes}, the answer is negative.
In~\ref{sec:functional-complexity-classes} we introduce the still standard, but much less talked about, \emph{functional} complexity classes,
...
in~\ref{chap:recursion-theory}, programming-language-like characterizations of decisional complexity classes 
...
Recall the definitions of complexity classes~\ref{def:turing-dtime} and~\ref{def:turing-dspace}.
...
We will study these in~\ref{sec:alternative-ptime}.
...
% Alternative definition of "P on unordered structures" defined in~\ref{def:inv-P}.
...
% Later in~\ref{sec:uniformity} we will restrict the family of (some standard) descriptions of the circuits
