...
In this chapter we analyze if we could, instead of creating a separate programming language
for each of complexity classes of interest, utilize results of the form 
``problem $P$ is \emph{complete} for class $C$ under
reductions in class $R$''.
...
\begin{theorem}\label{thm:cvp-pcomplete}
  The circuit value problem is \compL-complete for \compP~\cite{10.1145/990518.990519}.
...
\begin{definition}[\compL-reductions~{\cites[Definition~4.16]{10.5555/1540612}[Definition~4.14]{DRAFT10.5555/1540612}}]\label{def:logspace-reductions}
...
Some functional problems can are solvable in polynomial time with polynomially
...
\todo[inline]{Decide on which definition of FP to use}
...
% We accompany this class by our only notion of reducibility between functional problems:
...
\begin{definition}
  \todo[inline]{Here discuss definitions of reductions computed by circuit families}
\end{definition}


\subsection{Language for FL}
The class \complexity{FL} is precisely the class of function \complexityi{NC}{1}-reducible
to \compL, as we see in the below
\begin{theorem}[Obtaining \complexity{FL} from \compL and circuit reductions]\label{thm:fl-is-l-red}
  \(\complexity{FL} = \complexity{L}^\ast\)~\cite[Proposition~4.1]{COOK19852}
...
\subsection{Language for FP}
I don't have results here to cite\ldots

\subsection{Language for \complexity{FNP}}
Can we obtain the class \complexity{FNP} this way? from \complexity{FP} and an \complexity{NP}-complete
problem?  Not really! You get \(\complexity{FP}^{\complexity{NP}}\),
    which is a well-studied complexity class.
...
\begin{remark}[Bibliography]
An overview of problems complete for \compL:~\cite{COOK1987385}.
...
A detailed description of a problem complete for \compP and \compL-reductions to it is in~\cite{Kozen2006}.
\compP-completeness is defined formally e.g.\ in~\cites[Definition~6.25]{DRAFT10.5555/1540612}[Definition~6.28]{10.5555/1540612}.
\end{remark}
...
For this approach to succeed, we need to have a complete problem
for the class of interest. Not all complexity classes studied have known
complete problems.
...
studied in~\autoref{remark:unordered-structures}, is present in~\cite{dawar2012syntactic}.
For the class \complexity{BPP}, some discussion is in~\cite{35236}.
A ``less implicit'' characterization of BPP was studied in~\cite{lago2012higherordercharacterizationprobabilisticpolynomial}.
