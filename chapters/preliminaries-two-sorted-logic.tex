...
% Okolice Def. 2.2.2: Implicite zakładamy związek między \vec{X} oraz t\in X, więc nie dowolna struktura tylko
% coś w stylu że nie zachodzi t\in X dla liczb >= \vec{X}.
...
\begin{remark}
Note that \(X(i)\) and $\len{X}$ intuitively should be somehow related. 
