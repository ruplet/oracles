...
\begin{align*}
  ...
  \exists X \le t\ldotp \varphi
    &\;\;\text{stands for}\;\;
      \exists X\ldotp (\,\len{X} \le t \;\wedge\; \varphi), \\[2pt]
  \forall X \le t\ldotp \varphi
    &\;\;\text{stands for}\;\;
      \forall X\ldotp (\,\len{X} \le t \;\to\; \varphi).
\end{align*}
...
We don't claim that every formula can be transformed into an equivalent formula in some $\Sigma^{B}_{i}$.
% The limitation of shape of formulas here is not a~limitation on all the two-sorted formulas
% we can study.
In what follows, we will freely reason about arbitrary two-sorted formulas~---~we~will just not say that they are in $\Sigma^B_i$.
...
Note that if you used $t := \len{X}$, such a bounded quantifier would expand to $\exists X \le \len{X}$,
