...
Let $\mathbb{N}_2$ denote the natural numbers in binary notation.
  ...
  with $m$ normal and $n$ safe arguments.
...
\begin{definition}[{\cite{10.1007/978-3-540-30477-7_21}}~Neergaard's $\complexity{BC}_\varepsilon^{-}$ algebra]\label{def:bc-eps}
  ...
  \begin{enumerate}
    ...
    \item\label{itm:neergaard-recursion}
      ...
      \]
      Their \emph{safe affine course-of-value recursion} is the function
      $f : \mathbb{N}_2^{m+1,n}$ given by
      \[
      \begin{aligned}
        f(0,\vec{x};\vec{y}) &= g(\vec{x};\vec{y}),\\
        f(yb,\vec{x};\vec{y}) &=
          h_b \Bigl(y,\vec{x};\,f(\texttt{shift}(y,\delta_b),\vec{x};\vec{y})\Bigr)
      \end{aligned}
      \]
      for $b \in \{0,1\}$, where
      $\delta_b = \len{d_b(y,\vec{x};\,)}$ depends only on the normal
      arguments $(y,\vec{x})$. Here the recursive value
      $f(\texttt{shift}(y,\delta_b),\vec{x};\vec{y})$ is supplied to $h_b$ as
...
some of the propositions from~\cite{10.1007/978-3-540-30477-7_21}. We present two listings, just to give
a hint of how the corresponding functions look like; these are Haskell representations of functions from the
