...
\begin{enumerate}
    \item The purpose of~\autoref{chap:foundations} is to check in what programming paradigm/computation model,
    it will be the easiest for us to find characterizations. If imperative languages are hard, maybe consider functional.
    If computational classes for Turign machine are hard to capture, maybe other classes will  be easier to capture?
    \item The purpose of~\autoref{chap:formalized-semantics} is to examine reasoning about turing machines straight away,
    without any characterizations
    \item The purpose of~\autoref{chap:reductions} is to find out which classes we should actually focus on. Perhaps
    we can get some class from complete problem + reductions. Then we only need language for the lower class.
