...
\begin{enumerate}
    ...
    \item\label{itm:circ-nc} \(L \in \NCpoly{i}\) (Nick's class)  
          if each \(C_n\) uses only fan-in~2 \(\wedge\)-, fain-in~2 \(\vee\)-gates
          and fan-in~1 \(\neg\)-gates;

  \item\label{itm:circ-ac} \(L \in \ACpoly{i}\)  (Alternating circuits)
        if each \(C_n\) uses  
        unbounded fan-in \(\wedge\)-, unbounded fain-in \(\vee\)-gates and
        fan-in~1 \(\neg\)-gates;

  \item \label{itm:circ-tc}\(L \in \TCpoly{i}\)  (Threshold circuits)
        if each \(C_n\) uses unbounded fan-in \(\wedge\)-, unbounded fain-in \(\vee\)-gates,
        fain-in~1 \(\neg\)-gates and unbounded fain-in \emph{majority} gates
...
to have precisely one output node. We lift this requirement in~\autoref{def:faci}.
...
\begin{definition}[\FNCipoly, \FACipoly, \FTCipoly]\label{def:faci}
