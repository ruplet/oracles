...
complexity of randomized computation,
quantum computation and computation on real numbers (as opposed to bits) is considered.
...
\begin{remark}
In most complexity discussions we speak about decision problems, but real programs usually return
structured data rather than a single bit. To keep that practical viewpoint, we favour computational models
that natively handle producing structured output such as a binary string. We will still mention purely decisional
models whenever that is the natural formulation.
\end{remark}
...
The expressiveness of this model is inherently limited to Boolean-valued functions,
so for general functions it is strictly better for us to focus on transducers.
Yet, we consider some characterization of \complexity{REG}
in~\autoref{sec:descriptive-for-programming}. Please also see~\cite[Corollary~1 and Definition~in~section~2]{10.1145/321623.321625} for the characterization
of \compP{} by two-way multi-head pushdown automata and~\cite[Lemma~2]{Sudborough1977} for proof that \compL{}
languages are characterized by two-way multi-head finite automata.

Transducers extend automata to binary string outputs and have elegant characterizations.
In~\cite{bojańczyk2018polyregularfunctions},
four characterizations are given for the class of so-called \emph{polyregular} functions. The~definitions described there readily
...
provide basis for new programming languages.
% good picture:
% https://mimuw.edu.pl/~bojan/slides/slajdomat/teaching/2024transducers/MSO_transductions/?step=11

Because expressiveness of transducers remain poorly understood, it is usually unclear whether an arbitrary problem 
belongs to the class recognized by a given flavor of transducer. Writing programs as transducers
would rarely be useful for \todo{Ensure I'm consistent with this goal in the work}
certifying an arbitrary program to be in the desired complexity class, hence
for now we focus on other styles of characterization.
...
For a thorough overview of the computational complexity of imperative programs, see~\cite{KRISTIANSEN2004139}.

\subsection{Random-access Turing machines}
Random-access Turing machines are a version of Turing machines that allow to
read from and write to a specific memory cell in one step, if only the
address of the cell has been properly encoded on a special address tape.
Reasoning about computation in complexity classes such as logarithmic space or polynomial time is the same
for traditional Turing machines and random-access Turing machines. The choice of model starts to matter for
\emph{fine-grained} complexity classes such as the class of problems solvable
in linear or quadratic time --- since a standard Turing machine can only move its head one cell at a time,
obviously there are problems that will take it more than linear time, while being solvable in linear time by a
...
in~\autoref{remark:fine-grained-reductions};
...
also, the notion of \(\complexity{DLOGTIME}\)-uniformity explored in~\autoref{sec:uniform-nc1} is
only defined for random-access Turing machines.
% Remark: Random-access Turing machines are the intuition behind two-sorted logics in descriptive complexity!
...
A very rich
% \todo{there is nothing new for me
...
A particular logical characterization of \(\complexity{AC}^0\)~(\autoref{def:aci-faci}) will be important for us in~\autoref{chap:bounded-arithmetic}.
...
\(\complexity{FP}\)~(\autoref{def:fp}) and \(\complexity{FNP}\)~(\autoref{def:complexity-fnp}) are characterized.
Characterizations based on differential equations of various circuit complexity classes from
\(\complexity{FAC}^0\) to \(\complexity{FAC}^1\)~(\autoref{def:aci-faci}) 
...
Another classical paradigm is that of
% \todo{function algebras?}
general recursive functions, or equivalently the untyped
...
We treat these ideas in~\autoref{sec:recursion-theory}.
...
This will be discussed further in~\autoref{sec:linear-types}.
...
We will circle back
to this intuition while considering the \problem{PIGEON} computational problem in~\autoref{subsubsec:tfnp} and
the unprovability of the related pigeonhole principle in the weak theories studied in~\autoref{subsec:vac0-php}.
...
The forcing technique used originally by Cohen to prove the mentioned independence,
...
% zrobiłem cały przedmiot Set Theory, żeby zrozumieć forcing
