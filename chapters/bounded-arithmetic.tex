% Tym niemniej nadal nie załapałem jak to
% właściwie jest formalizowane, gdzie właściwie jest wymuszane że dowód jest tylko z bounded arithmetic.
% No i z tymi funkcjami też nie jest jasne o co chodzi, przynajmniej z tego co jest napisane na slajdach.
% Bo aby w ogóle pytać czy funkcja jest zdefiniowana dla każdego x, to trzeba ją najpierw zdefiniować -
% a jak ją zdefiujemy, to znaczy że jest zdefiniowana. No ale zakładam że definicja "na wejściu" to
% jakaś własność rozważanej funkcji, w stylu tego co jest napisane dla "x div y".
...
\begin{definition}[Functional complexity classes]
    \todo[inline]{Cook slightly differently defines functional complexity classes. He uses his two-sorted logic,
    and says that function is in FC when its poly-bounded and its graph is in C.
    }
\end{definition}

\begin{theorem}\label{thm:fc-is-fac0-closure}
    We obtain function classes for a few interesting classes (FL, FP, FTC0 etc.) from taking
    closure of complete problem by
    FAC0 reductions.~\cite[Section~IX.2.1]{Cook_Nguyen_2010}[Section~9B.1]{CookNguyenDraft}.
\end{theorem}

% \section{VAC0 and FAC0}
\section{\complexityi{VAC}{0} as a theory for \complexityi{FAC}{0}}\label{sec:vac0}

\subsection{Pigeonhole principle}\label{subsec:vac0-php}
% \todo[inline]{Here we cite the result that }
\begin{theorem}[Independence of \problem{PHP} from \complexityi{VAC}{0}]
    \[\texttt{VAC}^0 \nvdash \texttt{PHP}\]
    This is proved in~\cites[Corollary~7.21]{CookNguyenDraft}[Corollary~VII.2.4]{Cook_Nguyen_2010}.
\end{theorem}
...
\section{Complexity of algorithm vs complexity of proof}
See \cites[Section~IX.7.3]{Cook_Nguyen_2010}[Section~9G.3]{CookNguyenDraft} for
an open problem whether the recent breakthrough that binary integer division
is in \complexity{DLOGTIME}-uniform \complexityi{TC}{0}~\cite{HESSE2002695},
means that it can also be proved in the corresponding VTC0 theory.
...
% using Cobham's original characterization of \complexity{FP} with explicit bounds, as mentioned in~\autoref{sec:cobham-characterization}.
...
% As we observed in~\autoref{sec:complete-problems}, \complexityi{AC}{0} reductions are enough for us
...
% In 1985 Sam Buss PhD thesis was released \cite{Buss1986} introducing Bounded arithmetic.
