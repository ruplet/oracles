...
$y$, given $x$ as input. Such \emph{code extraction} from proofs in arithmetic is nothing new
and has been studied for decades, e.g.\ under the name of \emph{Kleene realizability}. However,
...
For the purpose of this thesis, we have implemented an example of code extraction from a proof tree
in the Lean proof assistant into a Lean function taking a binary string as input and outputting another
one --- the output of the function corresponding to the proof. The procedure is not general;
it is discussed with the file \texttt{BoundedArithmetic/Extraction.lean} below.
...
\begin{remark}
    ...
    we would need to \emph{formalize} the arithmetical theory in a computer, which we study in~\autoref{sec:formalization}.
\end{remark}
...
In the file $\texttt{BoundedArithmetic/V0.lean}$ we present\todo{Clean this file, now it's broken}
a way \compVZero{} can be formalized despite the $\texttt{Mathlib.ModelTheory}$ library for now only supporting
single-sorted logic.\footnote{We have initially started with extending this library to support many sorts,
but this was too much work for our time horizon.}. In the file $\texttt{BoundedArithmetic/VTC0.lean}$,
...
can be deeply embedded in the logic of a proof assistant. In the file
$\texttt{syntax-metaprogramming/roq-fol-proof-mode.v}$ we present a short demonstration
...
on the meta-level of Lean --- i.e.\ how to use metaprogramming in Lean 4 to obtain syntactical abstraction
