...
This is not trivial how to turn the concepts from bounded arithmetic into a programming language.
...
by the means of \emph{code extraction}: first, provide the user
a proof assistant with strenght limited to an appropriate arithmetical theory.
...
as of December 2025, no tool has been created that allow to extract code \emph{with complexity bounds}.
...
proofs in the theory into computational procedures that are both proven correct and have a strict
complexity bound, imposed by the proof being conducted in a bounded arithmetical theory.

This approach is very closely related to the notion of Curry-Howard, or ``proofs-as-programs''
...
into a program. We briefly discuss the expressiveness of intuitionisic bounded arithmetical
theories in~\autoref{remark:bounded-arith-biblio}. Note that for the purpose of code extraction,
we don't have to limit the whole reasoning to the intuitionistic fragment of the theory --- 
...
in the Lean proof assistant, to a Lean function taking a binary string as input, and outputting another
one --- the output of the function corresponding to the proof. The procedure is not general;
we discuss it in more detail as the file $\texttt{BoundedArithmetic/Extraction.lean}$ below.
...
of such a language would be limited by the lack of intesional expressiveness of
\complexity{Cob} and of characterizations from ICC\@. However, in~\cite{Li2025FeasibleMathematics}
the authors show how to apply relatively advanced programming techniques inside of
$\mathrm{IMP}(\complexity{PV})$.

\begin{remark}
    We will not introduce the definition of $\mathrm{IMP}(\complexity{PV})$ here, but will only note
    that it fundamentally relies on the notion of provability of theorems inside of an arithmetical theory.
