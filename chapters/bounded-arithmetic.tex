...
and the mathematical theories we should study to explore this connection further are called \emph{bounded arithmetics}.

We will start by looking at interesting theorems about one of such theories, just after
...
that we have used earlier. However, the difference will not matter in this work.
\end{definition}
...
This way, we obtain theories that foster both certifiation of complexity of an algorithm (if the proof of
correctness itself is feasible, see~\autoref{subsec:complexity-alg-proof})
and enable to prove theorems about the correctness of functions defined.
...
the theories $\mathrm{S}^1_2$ were introduce to capture reasoning about the polynomial-time hierarchy \complexity{PH}
...
Now, we want to formalize these arithmetic so that a computer can check our programs and ensure we didn't
go out of a given complexity at any point. I.e.\ to actually program on a computer and not in paper.
With such a programming language (and a formalization of arithmetic in general),
we are able to readily transfer a huge amount of results from paper to computer.
...
based on the equational theory $\complexity{PV}$ which is different to (and less interesting than)
...
\todo[inline]{Say how i use curry-howard correspondce to extract code from proofs as introduced in chapter linear logic}
