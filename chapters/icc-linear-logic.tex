...
in the language. The class \complexity{FL} has been captured by a variant of affine logic in~\cite{4276584};
it was also been studied in~\cite{10.1007/11874683_40},~\cite{DaLagoSchopp10} and~\cite{mazza:LIPIcs.CSL.2015.24}.
The class \complexity{FP} was characterized in~\cite{Leivant93}. Most importantly for us, the programming
language \texttt{IntML}
was introduced in\cite{DALLAGO2016150}.

\begin{remark}
The formal bridge between the theories of linear types and the different kinds of linear logics is the
Curry-Howard correspondence,
also known as ``propositions as types'', or ``proofs as programs''.
The depth of connections between logic, type systems and complexity theory is astonishing
and already well-explored in the literature, e.g.\ \cite{BenedettiPhd}. We will not repeat these definitions here.
...
\begin{remark}\label{remark:haskell-linear}
   ...
   As we see in~\autoref{lst:haskell-linear-example}, we really get lots.
\end{remark}


\subsection{IntML}\label{sec:intml}
An implementation of IntML is available on
