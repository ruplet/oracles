(* (Linear) BC evaluator: evaluator of Bellantoni-Cook's function algebra for PTIME.
    ...
    Copyright 2003,2004 Peter M�ller Neergaard (e-mail: turtle@achilles.linearity.org)
    and Harry Mairson.
...
Ong and extended by M�ller Neergaard).  The interpreter illustrates
that the extended linear fragment can be evaluated in logarithmic
space as described by M�ller Neergaard in ``A Functional Language for
Logspace''.
